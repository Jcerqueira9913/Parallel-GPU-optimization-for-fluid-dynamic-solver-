...
//(threads em x, em y, em z) (32,2,2)
#define tz 2
#define ty 2
#define tx 32
#define THREADS_PER_BLOCK 128
//define a quantidade de blocos li_solve
#define dx 3
#define dy 84
#define dz 84
//define blocos em x para funçoes fora o solver
#define dg 6
...
//define os blocos que são necessários à redução em numero
...
//CUDA KERNELS USADOS NAS DIVERSAS FUNÇÕES:
	...
	//kernel add_source
	__global__ void add_source_kernel(float *x, float *s, float dt, int size) {
	...
	//Kernel's lin_solve
		//funçao mapeamento shared memory
		__device__ int IX_sh(int i,int j,int k) {
			return (i>>1)+(j*(tx+1))+(k*(ty+2)*(tx+1));
		}

		//kernel's para passos black/red
		__global__ void red(int M, int N, int O, float *x, float *x0, float a, float c, float *max_c) {
			__shared__ float shared_max[THREADS_PER_BLOCK];
			__shared__ float sh_x[(tz+2)*(ty+2)*(tx+1)];

			//indices i,j,k i usados de forma a andar de 2 em 2 sem desperdiçar threads assim so precisamos de metade na dim i
			...
			if (i<=M+2 && j<=N+2 && k<=O+2) {
				...
				if (lk==tz) {
					if (k+1<O+2)
						sh_x[IX_sh(li,lj,lk+1)]=x[IX(i,j,k+1)];//adicionado a preto
			...
			if (i<=M && j<=N && k<=O){
				float old_x = x[idx];
		  		...
		  		float novo=(x0[idx]+ a * (sh_x[IX_sh(li - 1, lj, lk)] + sh_x[IX_sh(li + 1, lj, lk)] +
		...
		__global__ void black(int M, int N, int O, float *x, float *x0, float a, float c, float *max_c) {
			//analogo a red mas diferença no indice i/li
			__shared__ float shared_max[THREADS_PER_BLOCK];
			__shared__ float sh_x[(tz+2)*(ty+2)*(tx+1)];
			...
			if (tid == 0) {
				//verifica o valor já presente no array
		...
		__global__ void reduce_max(float *block_max, float *global_max) {
			...
			//id local
		  	...
		  	// Redução para encontrar o máximo global
			...
			// A primeiro thread do bloco armazena o máximo final na memória global
			if (tid == 0) {
...
//FUNÇÕES DE CPU
	...
	void add_source(int M, int N, int O, float *x, float *s, float dt) {
	  ...
	  add_source_kernel<<<(int) (size+511)/512, 512>>>(x, s, dt,size);
	}

	// Set boundary conditions
	void set_bnd(int M, int N, int O, int b, float *x) {
		loop1<<<dim3 (dg,dy,1),dim3 (tx,ty,1)>>>(x,b,M,N,O);

		loop2<<<dim3 (42,42,1),dim3 (4,4,1)>>>(x,b,M,N,O);

		loop3<<<dim3 (dg,dy,1),dim3 (tx,ty,1)>>>(x,b,M,N,O);
	}
	...
	void lin_solve(int M, int N, int O, int b, float *x, float *x0, float a, float c) {
	  ...
	  do {
	    ...
	    red<<<blocks, threads>>>(M, N, O, x, x0, a, c, aux_max_c);
  		//CUDA_CHECK_ERROR();
	    black<<<blocks, threads>>>(M, N, O, x, x0, a, c, aux_max_c);
  		//CUDA_CHECK_ERROR();
